...
// services/notificationService.js — FINAL 100% WORKING (Copy-Paste This)
...
exports.deliver = async (notification, session = null) => {
  ...
  try {
    ...
    // 1. Get school with communication settings
    const school = session
      ? await School.findById(notification.schoolId).session(session)
      : await School.findById(notification.schoolId);
    ...
    const phone = notification.data.recipientPhone || recipient.phoneNumber;
    const email = recipient.email;

    const message = notification.message.substring(0, 160);
    const schoolName = school.name || 'Your School';

    // 3. SMS — FROM SCHOOL'S SENDER NAME
    if (phone && school.smsPackActive) {
      try {
        ...
        deliverySuccess = true;
        logger.info(`SMS sent from ${senderName} to ${phone}`);
      } catch (smsErr) {
        deliveryErrors.push(`SMS: ${smsErr.message}`);
        logger.error(`SMS failed: ${smsErr.message}`);
      }
    }

    // 4. EMAIL — FROM SCHOOL'S EMAIL
    if (email) {
      try {
        const transporter = nodemailer.createTransport({
          host: 'smtp.gmail.com',
          ...
          secure: true,
          auth: {
            user: school.communication?.emailFrom || process.env.EMAIL_USER,
            pass: school.communication?.emailPass || process.env.EMAIL_PASS
          },
          tls: { rejectUnauthorized: false }
        });

        await transporter.sendMail({
          from: `"${school.communication?.emailName || school.name}" <${school.communication?.emailFrom || process.env.EMAIL_USER}>`,
          to: email,
          subject: notification.title,
          html: `<p>${notification.message.replace(/\n/g, '<br>')}</p>`
        });
        deliverySuccess = true;
        logger.info(`Email sent from ${school.communication?.emailFrom || 'your Gmail'}`);
      } catch (emailErr) {
        deliveryErrors.push(`Email: ${emailErr.message}`);
